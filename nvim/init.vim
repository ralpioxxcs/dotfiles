"  __  ____   __  _  _ ___ _____   _____ __  __ 
" |  \/  \ \ / / | \| | __/ _ \ \ / /_ _|  \/  |
" | |\/| |\ V /  | .` | _| (_) \ V / | || |\/| |
" |_|  |_| |_|   |_|\_|___\___/ \_/ |___|_|  |_|
"                                              
"-----------------------------------------------

source $HOME/.config/nvim/vim-plug/plugins.vim
source $HOME/.config/nvim/general/settings.vim
source $HOME/.config/nvim/general/functions.vim
source $HOME/.config/nvim/general/key_bindings.vim
source $HOME/.config/nvim/themes/config.vim
source $HOME/.config/nvim/themes/airline.vim

" Plugin configs
source $HOME/.config/nvim/plugin-config/fzf.vim
source $HOME/.config/nvim/plugin-config/vim-multiple-cursors.vim
source $HOME/.config/nvim/plugin-config/vim-clang-format.vim
source $HOME/.config/nvim/plugin-config/tagbar.vim
source $HOME/.config/nvim/plugin-config/coc-nvim.vim
source $HOME/.config/nvim/plugin-config/Ultisnips.vim
source $HOME/.config/nvim/plugin-config/nerdtree.vim
source $HOME/.config/nvim/plugin-config/vim-cpp-enhanced-highlight.vim
